...
export default function Trends() {
  ...
  const weekSlot = useMemo(() => {
    ...
    return [
      { slot: 'Weekday', Activation: sum(actWeekday, 'ActivationAmount'), Redemption: sum(redWeekday, 'RedemptionAmount') },
      { slot: 'Weekend', Activation: sum(actWeekend, 'ActivationAmount'), Redemption: sum(redWeekend, 'RedemptionAmount') }
    ]
  }, [activationRows, redemptionRows])
      ...
      <Card title="Week-slot Overview" subtitle="Weekday vs. Weekend — Activation and Redemption amount, ₹ Lacs">
        {!hasData ? (
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={weekSlot} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke={COLORS.gridline} vertical={false} />
